...
public class OfferteInviateView {
    ...
    private void createUI() {
        ...
        Label title = new Label("Le mie Offerte Inviate");
        ...
        cbStato.getItems().addAll("Tutti gli stati", "inviata", "accettata", "rifiutata", "annullata");
        ...
        colAzioni.setCellFactory(param -> new TableCell<>() {
            ...
            {
                btnAnnulla.setStyle("-fx-background-color: #ff6b6b; -fx-text-fill: white; -fx-font-weight: 700; -fx-background-radius: 8; -fx-padding: 4 8;");
    ...
    private void reloadData() {
        try {
            ...
            masterData.setAll(controller.getOfferteInviateByUtente(matricola));
    ...
    private void applyFilters() {
        ...
        filtered.setPredicate(o -> {
            ...
            boolean searchMatch = (search.isBlank() || (o.getCodiceAnnuncio() != null && o.getCodiceAnnuncio().toLowerCase().contains(search)));
    ...
    private void annullaOfferta(Offerta offerta) {
        ...
        Alert conferma = new Alert(Alert.AlertType.CONFIRMATION, "Sei sicuro di voler annullare l'offerta " + offerta.getCodiceOfferta() + "?", ButtonType.YES, ButtonType.NO);
        conferma.setHeaderText("Annulla Offerta");
        conferma.showAndWait().ifPresent(response -> {
            if (response == ButtonType.YES) {
                try {
                    if (controller.eliminaOfferta(offerta.getCodiceOfferta())) {
                        reloadData();
                    } else {
                        warn("Errore durante l'annullamento dell'offerta.");
                    }
                } catch (SQLException e) {
                    warn("Errore annullamento offerta: " + e.getMessage());
                }
            }
        });
    }
    ...
    private void mostraDettaglioOfferta(Offerta offerta) {
        ...
        grid.add(l("Codice Offerta:"), 0, 0);
        grid.add(valueLabel.apply(offerta.getCodiceOfferta()), 1, 0);
    
        grid.add(l("Annuncio:"), 0, 1);
        grid.add(valueLabel.apply(offerta.getCodiceAnnuncio()), 1, 1);
    
        grid.add(l("Tipologia:"), 0, 2);
        grid.add(valueLabel.apply(offerta.getTipo()), 1, 2);
    
        grid.add(l("Stato:"), 0, 3);
        grid.add(valueLabel.apply(offerta.getStato()), 1, 3);
    
        grid.add(l("Data:"), 0, 4);
        grid.add(valueLabel.apply(offerta.getData() != null ? new SimpleDateFormat("dd/MM/yyyy").format(offerta.getData()) : "N/A"), 1, 4);
    
        if ("vendita".equals(offerta.getTipo()) && offerta.getPrezzoOfferto() != null) {
            grid.add(l("Prezzo offerto:"), 0, 5);
            grid.add(valueLabel.apply("€" + String.format("%.2f", offerta.getPrezzoOfferto())), 1, 5);
        }
        ...
        if (offerta.getMessaggio() != null && !offerta.getMessaggio().isBlank()) {
            ...
            grid.add(l("Messaggio:"), 0, 6);
            grid.add(taMessaggio, 0, 7, 2, 1);
        }
